...
def image_stream(datapath, image_size=[384, 512], intrinsics_vec=[320.0, 320.0, 320.0, 240.0], stereo=False):
    ...
    ht0, wd0 = [480, 640]
    print(datapath)
    images_left = sorted(glob.glob(os.path.join(datapath, 'image_left/*.png')))[:50]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--stereo", action="store_false")
    parser.add_argument("--disable_vis", action="store_true")
    parser.add_argument("--plot_curve", action="store_true")
    parser.add_argument("--id", type=int, default=-1)

    parser.add_argument("--beta", type=float, default=0.3)
    parser.add_argument("--filter_thresh", type=float, default=.1)
    parser.add_argument("--warmup", type=int, default=12)
    parser.add_argument("--keyframe_thresh", type=float, default=.1)
    ...
    for scene in test_split:
        ...
        scenedir ="/home/parth/NEURFL/NEW_DROID_SLAM/DROID-SLAM/data/abandonedfactory_Easy_image_left/abandonedfactory/Easy/P011/"

        print("SCENE",os.path.join(scenedir, 'depth_left/*.npy'))
        
        for (tstamp, image, intrinsics) in tqdm(image_stream(scenedir, stereo=False)):
            print(tstamp)
        ...
        results = evaluator.evaluate_one_trajectory(
            traj_ref[:50], traj_est, scale=True, title=scenedir[-20:].replace('/', '_'))
